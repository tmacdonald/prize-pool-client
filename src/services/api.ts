...
export async function getBallots(poolId: string, requestInfo: RequestInit = {}): Promise<Ballot[]> {
  const response = await fetch(`${serverUrl}/api/v1/pools/${poolId}/ballots`, requestInfo);
  const ballots = await response.json() as Ballot[];
  return ballots;
}

export async function createMatches(poolId: string) {
  const response = await fetch(`${serverUrl}/api/v1/pools/${poolId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: 'sps-cake-walk', numPrizes: 10, areMatchesSet: true })});
